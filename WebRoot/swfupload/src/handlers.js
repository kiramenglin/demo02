...
function initSwfupload(url,sizeLimit,types,typsdesc,uploadLimit){
		...
		var settings={
			...
			flash_url : "/demo02/swfupload/vendor/swfupload/swfupload.swf",
			...
			// Button Settings
			button_image_url : "http://localhost:8080/demo02/images/select-files-zh_CN.png",
	...
	try{
		//等 1秒后初始化 swf
		setTimeout(function (){swfu = new SWFUpload(settings);},2000);
		//alert("hehe2");
	}catch (ex) {
...
function swfUploadLoadFailed() {
	...
	var message='<div id="divAlternateContent" style="background-color: #FFFF66; text-align:center;">'
			+'[url=http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash]<font color="red">请安装或者升级您的Flash插件!</font>[/url]</div>';
	addErrorMsg(message,false);
}
...
function uploadSuccess(file, serverData) {
	try {
		...
		if (data.status == 0 || data.status == "0") {
			$("#"+file.id).find(".ke-img").attr("src","http://localhost:8080/demo02/"+data.message);
			progress.setStatus("上传成功");
			progress.toggleCancel();
			if(insertAllStatus){
				if(swfu.getStats().successful_uploads>0){
					$("#"+swfu.customSettings.insertAll_Id).bind("click",insertAllevent);
				}
...
function addReadyFileInfo(fileid,fileName,message){
	var photoDiv='<div class="ke-inline-block ke-item" id="'+fileid+'">'
			...
			+'	<img src="images/image.png" class="ke-img" data-status="-1" width="80" height="80/>'
