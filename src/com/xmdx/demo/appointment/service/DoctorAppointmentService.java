...
public class DoctorAppointmentService extends BusinessServices {
	...
	public int agree(ActionContext ac) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("enter agree");
		int state = 1;
		int result = 0;
		
		String appointmentid = ac.getHttpRequest().getParameter("APPOINTMENT_ID");
		System.out.println("appointmentid="+appointmentid);

		DBDYPO po = new DBDYPO(tableName,keyField);
		po.set("APPOINTMENT_ID", appointmentid);
		DBDYPO apo[] = DBDYDao.selectByID(ac.getConnection(), po);
		if(apo[0].get("STATE").toString().equals("0")){
		po.set("STATE", state);
		result = DBDYDao.update(ac.getConnection(), po);
		if(0 == result) {
			...
			setMessage(ac, "状态已更改，请不要再尝试!");
		}
		return CONST_RESULT_AJAX;
	}
	
	public int reject(ActionContext ac) throws Exception {
		// TODO Auto-generated method stub
		System.out.println("enter reject");
		int state = 2;
		int result = 0;
		...
		po.set("APPOINTMENT_ID", appointmentid);
		DBDYPO[] apo = DBDYDao.selectByID(ac.getConnection(), po);
		System.out.println("state="+apo[0].get("STATE").toString());
		if(apo[0].get("STATE").toString().equals("0")){
		po.set("STATE", state);
		result = DBDYDao.update(ac.getConnection(), po);
		if(0 == result) {
	...
	public int read(ActionContext ac) throws Exception {
		...
		ac.setStringValue(CONST_FORMNAME, "com/xmdx/demo/appointment/doctor_appointment_read.html");
...
//		
		...
		if(pop.length==0)
		{
			...
			ac.setStringValue(CONST_FORMNAME, "com/xmdx/demo/appointment/doctor_appointment_main.html");		
			return CONST_RESULT_SUCCESS;
		}
...
//		return CONST_RESULT_SUCCESS;
		...
		for(int i = 0; i< polist.size();i++){
			...
			po1.set("APPOINTMENT_ID", pop[i].get("APPOINTMENT_ID").toString());
...
//		return CONST_RESULT_SUCCESS;
		...
		{
			ac.setStringValue("SIZE", String.valueOf(pop.length));
			ac.setStringValue(CONST_FORMNAME, "com/xmdx/demo/appointment/doctor_myappointment_main.html");		
			return CONST_RESULT_SUCCESS;
		}
		else{
		...
		for(int i = 0; i< polist.size();i++,n++){
			...
			po1.set("APPOINTMENT_ID", pop[n].get("APPOINTMENT_ID").toString());
